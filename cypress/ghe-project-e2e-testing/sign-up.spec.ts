...
describe("sign up and role", () => {
  ...
  beforeEach("add api listener", () => {
    ...
    cy.intercept("POST", "/api/v1/requestuser/action/Approve").as(
      "approveNewUser"
    );
    cy.intercept("POST", "/api/v1/requestuser/action/Reject").as(
      "rejectNewUser"
    );
  ...
  context("agency account", () => {
    ...
    it("can delete new project", function () {
      ...
      cy.deleteProject(this.userName, false);
    });
    ...
    it.skip("can delete agency user", function () {
  ...
  context("notary office account for approval", () => {
    ...
    context(
      "log in by notary office account and check for legal support logic",
      () => {
        beforeEach("log in as notary office", function () {
          cy.logInCmd(`${this.prefix}.${this.userName}`, this.password);
          ///api/v1/legalsupport/update-document
          cy.intercept("POST", "/api/v1/legalsupport/update-document").as(
            "updateDocument"
          );
        });
        ...
        it("create new legal support and check for creation logic", function () {
          ...
          cy.get("p")
            .contains(`Field 'Customer address' is required`)
            .should("exist");
          ...
          cy.get('input[name="customerName"]').type(
            `customerName${_randomNumber}`
          );
          cy.get('input[name="customerIdentity"]').type(
          ...
          cy.get('input[name="customerAddressId"]')
            .parent()
            .click()
            .then(() => {
          ...
          for (let _i = 1; _i <= 4; _i++) {
            cy.get('input[name="customerMaritalStatus"]')
              .parent()
              .click()
              .then(() => {
        ...
        it.skip("delete notary office account", function () {
          cy.deleteUserByAccountName(this.accountName);
        });
      }
    );

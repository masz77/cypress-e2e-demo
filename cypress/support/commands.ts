...
Cypress.Commands.add("setUpAliasesForMaterialTab", function () {
  ...
  cy.url().then((text) => {
    ...
    cy.intercept("POST", `api/v1/material/project/${projectID}`).as(
      "modifyingProject"
    );
    //after saving FE will try to re-query the page
    cy.intercept(
      "GET",
      `api/v1/material/project?p=0&projectId=${projectID}&ps=10`
    ).as("thenReQueryingThePage");
    //api/v1/material/project delete route
    cy.intercept("DELETE", `api/v1/material/project`).as(
      "deleteMaterialOfAProject"
    );
    // api/v1/material/project/21/copy copy material from existing project
    cy.intercept("POST", `api/v1/material/project/${projectID}/copy`).as(
      "copyMaterialFromOtherProject"
    );
  });
});
...
Cypress.Commands.add(
  ...
  function (_alias, statusCode, haveAnotherOKCheck) {
...
Cypress.Commands.add("deleteProject", function (projectName, havePopup) {
  //navigate to project
  cy.navigateTo("project");
  cy.searchFor(projectName);
  cy.isExistInRow(projectName, true);

  cy.pressDeleteButtonThenOK("@deleteProject", 200, havePopup);
...
Cypress.Commands.add("createNotaryOfficeUser", function () {
  ...
  cy.get('div[role="status"]')
    .contains("please wait for approval")
    .should("exist")
    .and("be.visible");
});
...
Cypress.Commands.add("checkBoxShouldHaveLength", function (_numberOfCheckBox) {
  cy.get('input[type="checkbox"]')
    .should("have.length", _numberOfCheckBox)
...
Cypress.Commands.add(
  ...
  function (accountName, phone, prefix, userName, password, mode) {
    ...
    cy.get('[data-test-id="phone"]').should(
      "have.attr",
      "aria-invalid",
      "true"
    );
...
Cypress.Commands.add("searchFor", function (searchText) {
  ...
  cy.get('div[data-test-id="searchDiv"]')
    .click()
    .then(() => {
...
Cypress.Commands.add(
  ...
  function (test_id, test_oldPwd, test_newPwd) {
...
Cypress.Commands.add("addNew", (InteriorOrExterior) => {
  ...
  cy.get('input[type="file"]').selectFile("@uploadImg", {
    force: true,
    // encoding: 'binary',
  });
  //button contain ok
...
Cypress.Commands.add("setUpListener", function setUpListener(inOrEx) {
  ...
  cy.url().then((text) => {
    ...
    cy.intercept("POST", `/api/v1/${inOrEx}view/project/${projectID}`).as(
      "saveAsNew"
    );
    ...
    cy.intercept(
      "GET",
      `/api/v1/${inOrEx}view?p=0&projectId=${projectID}&ps=10`
    ).as("refreshPage");
  });
  ...
  try {
    if (expected == "disabled") {
      for (let i = 0; i < _selector.length; i++) {
        ...
        cy.get(`a[data-test-id="${_element}"]`).should(
          "have.attr",
          "aria-disabled",
          "true"
        );
...
Cypress.Commands.add("isProjectPropertiesEnabled", () => {
  try {
    //assert material, in/exteriors are disabled on newly added project
    cy.get('a[data-test-id="material"]').should(
      "not.have.attr",
      "aria-disabled"
    );
    cy.get('a[data-test-id="interiors"]').should(
      "not.have.attr",
      "aria-disabled"
    );
    cy.get('a[data-test-id="exteriors"]').should(
      "not.have.attr",
      "aria-disabled"
    );
    cy.get('a[data-test-id="sharing"]').should(
      "not.have.attr",
      "aria-disabled"
    );
  } catch (error) {}
...
Cypress.Commands.add("visitTheMainPage", () => {
  const _mainPage: string = `${Cypress.config().baseUrl}sign-in`;
  cy.visit(_mainPage, {
    onBeforeLoad(win) {
